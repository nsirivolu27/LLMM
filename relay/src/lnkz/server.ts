...
app.use(createHostValidator(allowedHosts));
...
app.get("/health", (_request, response) => {
  response.json({
    ...
    protocol: "LNKZ REST",
    contextForwarding: { enabled: Boolean(process.env.LNKZ_MCP_CONTEXT_SECRET?.trim()) },
...
const httpServer = app.listen(port, host, (error?: Error) => {
  ...
  if (!process.env.LNKZ_API_KEY?.trim()) {
    console.warn("[server] LNKZ_API_KEY is not set: the REST API is unauthenticated.");
  }
});
