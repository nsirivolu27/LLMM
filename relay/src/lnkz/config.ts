...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): AppConfig {
  ...
  validateContextSecret(env.LNKZ_MCP_CONTEXT_SECRET);
  ...
  const apiKeyRequired = Boolean(apiKey) || authMode === "multi-key" || !allowUnauthenticated;
...
function validateContextSecret(value: string | undefined): void {
  if (!value) return;
