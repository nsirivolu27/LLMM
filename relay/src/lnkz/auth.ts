...
export function createHostValidator(allowed: string[]): (request: Request, response: Response, next: NextFunction) => void {
  const normalized = new Set(allowed.map((host) => host.toLowerCase()));
  return (request, response, next) => {
    if (!normalized.size) {
      next();
      return;
    }
    const host = request.header("host")?.trim().toLowerCase();
    if (!host || !normalized.has(host)) {
      response.status(403).json({ error: "Host is not allowed." });
      return;
    }
    next();
  };
