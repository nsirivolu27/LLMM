...
`LNKZ_MCP_CONTEXT_SECRET` is needed only when this relay calls a mutually trusted downstream MCP
target and must preserve authenticated actor context. Generate at least 32 random bytes and store
the value in the deployment secret manager. Every mutually trusting process must use the same
value. Never expose it through client configuration, logs, health checks, or committed environment
files. The separate `lnkz-mcp` REST adapter uses `LNKZ_BASE_URL` and `LNKZ_API_KEY` instead.
...
- `GET /health` returns the REST service version and connector configuration without secrets.
- Exercise protected `/api/*` routes with an authorized bearer key.
- `/share/:token` is intentionally bearer-based, rate-limited, and uncached.
- Never cache `/api/*` or `/share/*` at a reverse proxy.
